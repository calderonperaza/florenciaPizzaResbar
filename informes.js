...
function proxy(fn, context) {
    for (
        var len = arguments.length, args = Array(len > 2 ? len - 2 : 0), key = 2; key < len; key++
    ) {
    ...
    return function() {
        for (
            var len2 = arguments.length, args2 = Array(len2), key2 = 0; key2 < len2; key2++
        ) {
...
if ($.fn) {
    $.fn.yearpicker = function jQueryYearpicker(option) {
        for (
            var len = arguments.length, args = Array(len > 1 ? len - 1 : 0), key = 1; key < len; key++
        ) {
...
$(".yearpicker-body").on("mousedown", function(e) {
    ...
    // vm.$nextTick(() => this.$refs.input.focus())
    setTimeout(function() {
